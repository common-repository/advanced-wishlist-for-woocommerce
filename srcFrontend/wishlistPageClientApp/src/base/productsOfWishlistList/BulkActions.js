...
const BulkActions = (props) => {
    const {bulkActions, onBulkActionApply, isDesktop} = props;
    ...
    return (<Box component="span"
                 ...
                 alignItems="center"
                 sx={{height: isDesktop ? 50 : 'auto', margin: 2, gap: 2, flexWrap: isDesktop ? 'nowrap' : 'wrap'}}
    >
        <Select
            sx={{width: 150, ...(!isDesktop && {flexBasis: '40%'})}}
            variant="filled"
            value={action}
            label="Action"
            onChange={(e) => {
            ...
            // https://stackoverflow.com/a/63259945
            MenuProps={{
...

        {
            processing
                ? <CircularProgress/>
                : <Button
                    sx={{width: 100, height: 1, ...(!isDesktop && {flexBasis: '50%', marginLeft: 'auto', minHeight: 40})}}
                    variant="contained"
        ...
        <Button
            sx={{height: 1, ...(!isDesktop && {width: '100%', flexBasis: '100%', minHeight: 40})}}
            variant="contained"
